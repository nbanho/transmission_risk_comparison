...
sample_co2_ch1 <- sample(co2_distr_ch1$co2, 1000, replace = TRUE, prob = co2_distr_ch1$prob) #sample
sample_f_ch1 <- tibble(co2 = sample_co2_ch1, f = ((co2-C_o)/C_a)/100000) %>% 
  dplyr::select(-co2)
...
#descdist(x_ch2, discrete = FALSE) #gamma distribution fits well
...
sample_f_ch2 <- tibble(co2 = sample_co2_ch2, f = ((co2-C_o)/C_a)/100000) %>% 
  dplyr::select(-co2)
...
#descdist(x_tz, discrete = FALSE) #normal distribution fits well
#fitdistr(x_tz, "normal")

y_tz <- dnorm(x, mean = 648, sd = 209)
x_tz_norm <- data.frame(cbind(x,y_tz))

x_tz_norm %>% 
  ggplot(aes(x=x,y=y_tz)) +
  geom_line(color= "red")

#tz_fit_norm <- fitdist(x_tz, "norm") #different fitting function
...
sample_f_tz <- tibble(co2 = sample_co2_tz, f = ((co2-C_o)/C_a)/100000) %>% 
  dplyr::select(-co2)
...
#descdist(x_sa, discrete = FALSE) #normal distribution fits well
#fitdistr(x_sa, "normal")

y_sa <- dnorm(x, mean = 1457, sd = 596)
x_sa_norm <- data.frame(cbind(x,y_sa))

x_sa_norm %>% 
  ggplot(aes(x=x,y=y_sa)) +
  geom_line(color= "red")

#sa_fit_norm <- fitdist(x_sa, "norm") #different fitting function
#plot(sa_fit_gamma) #plots comparison

co2_distr_sa <- data.frame(co2 = seq(400, 3000, .1)) %>% 
  mutate(prob = dnorm(co2,1457,596))

sample_co2_sa <- sample(co2_distr_sa$co2, 1000, replace = TRUE, prob = co2_distr_sa$prob) #sample
sample_f_sa <- tibble(co2 = sample_co2_sa, f = ((co2-C_o)/C_a)/100000) %>% 
  dplyr::select(-co2)
...
### hier noch SD neu berechnen
dq <- function(x) {
  dtrunc(x, spec = "st", a = 0, b = 300, mu = q, sigma = 1.235, nu = 1) #sigma aus Escombe
...
#I
prev_ch <- (0.46 + 8.23)/200000 #decimal; values for age group 10-14 and 15-19 over the last two years, https://www.bag.admin.ch/bag/de/home/zahlen-und-statistiken/zahlen-zu-infektionskrankheiten.exturl.html/aHR0cHM6Ly9tZWxkZXN5c3RlbWUuYmFnYXBwcy5jaC9pbmZyZX/BvcnRpbmcvZGF0ZW5kZXRhaWxzL2QvdHViZXJrdWxvc2UuaHRt/bD93ZWJncmFiPWlnbm9yZQ==.html

prev_sa <- 30000/59000000 #https://worldhealthorg.shinyapps.io/tb_profiles/?_inputs_&entity_type=%22country%22&lan=%22EN%22&iso2=%22ZA%22

prev_tz <- 1177/61741120 
