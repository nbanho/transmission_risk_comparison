...
prevalence <- read_csv("data-clean/WHO-COVID-19-global-data.csv", col_types = cols(
...
prevalence <- prevalence %>% 
  filter(Date_reported >= as.Date("2020-03-01") & Date_reported <= as.Date("2022-12-31")) 
...
```{r smoothing}
# CH
## together old dataset
set.seed(1343)

dat_ch_old <- data.frame(
  x = 1:length(ch_old$co2),
  y = ch_old$co2
)

loessData_ch_old <- data.frame(
  x = 1:length(ch_old$co2),
  y = predict(loess(y~x, dat_ch_old, span = 0.1)),
  method = "loess()"
) %>% 
  mutate(school = "Switzerland")

ggplot(loessData_ch_old, aes(x, y)) + 
  geom_point(dat = dat_ch_old, aes(x, y), alpha = 0.2, col = "red") +
  geom_line(col = "blue") +
  facet_wrap(~method) +
  theme_bw(16)
...
loessData_ch_old %>% #density plot ch
  ggplot(aes(x = y)) +
  geom_density(alpha = .2, kernel = "gaussian", adjust = 3) +
  scale_x_continuous(expand = c(0,0)) +
  scale_color_brewer(palette = "Set2") +
...
co2_distr_ch <- data.frame(co2 = seq(400, 4000, .1)) %>% 
  mutate(prob = dtrunc(co2, spec = "gamma", a = 400, b = 4000, shape = 4.783, rate = 0.0044)) %>%
...
descdist(x_ch_old, discrete = FALSE) #gamma distribution fits well
fitdistr(x_ch_old, "normal") #get parameters for the gamma distribution

co2_distr_ch_old <- data.frame(co2 = seq(400, 4000, .1)) %>% 
  mutate(prob = dtrunc(co2, spec = "norm", a = 400, b = 4000, mean = 1067, sd = 376)) %>%
  mutate(country = "Switzerland")

co2_distr_ch_old %>% #plot for the fitted distribution (not used in the final paper)
  ggplot(aes(x=x,y=prob)) +
  geom_line(color= "red") +
  labs(x = expression(CO[2]*" (ppm)"), y = "Density") +
  theme_bw2() +
...
sample_co2_ch_old <- sample(co2_distr_ch$co2, 3000, replace = TRUE, prob = co2_distr_ch$prob) #sample co2 for dataframe later
sample_f_ch_old <- tibble(co2 = sample_co2_ch, f = ((co2-C_o)/C_a)/1000000) %>% #sample f for dataframe later
  dplyr::select(-co2)

####histogram / density comparison (plot in supplements) OLD
hist(loessData_ch_old$y, # histogram
 col="#FFA07A", # column color
 border="black",
 ...
 xlim = c(400,2500),
 ylim = c(0,0.0015))
lines(co2_distr_ch_old, # density plot
...
plot_co2_ch <- ch %>% 
  ...
  theme_bw2()+ 
  stat_function(fun = function(x) dtrunc(x, spec = "norm", a = 400, b = 4000, mean = 800, sd = 600), size = 1) +
  ggtitle("Switzerland") +
...
sample_co2_ch1 <- sample(co2_distr_ch1$co2, 3000, replace = TRUE, prob = co2_distr_ch1$prob) #sample co2 sample f
...
sample_co2_ch2 <- sample(co2_distr_ch2$co2, 3000, replace = TRUE, prob = co2_distr_ch2$prob) #sample
...
co2_distr_sa <- data.frame(co2 = seq(400, 4000, .1)) %>% 
  mutate(prob = dtrunc(x, spec = "gamma", a = 400, b = 4000, shape = 5.72, rate = 0.0039)) %>% 
...
plot_co2_sa <- sa %>% 
  ...
  coord_cartesian(xlim = c(400,3500))+
  theme_bw2()+
  stat_function(fun = function(x) dtrunc(x, spec = "gamma", a = 400, b = 4000, shape = 5.72, rate = 0.0039), size = 1) +
  ggtitle("South Africa") +
  theme(plot.title = element_text(hjust = 0.5))
...
co2_data <- rbind(co2_distr_ch,co2_distr_sa,co2_distr_tz)
...
plot_grid(plot_co2_ch,plot_co2_tz,plot_co2_sa, ncol = 2) #plot in the paper
...
I_ch_fix <- prev_sa*n_ch
I_sa_fix <- prev_sa*n_sa
I_tz_fix <- prev_sa*n_tz

# I SARS 
prevalence_ch <- prevalence %>% 
  filter(Country == "Switzerland") %>% 
   group_by(week = floor_date(Date_reported, "week")) %>% 
              summarise(New_cases = sum(New_cases)) %>% 
  summarise(low = quantile(New_cases, probs = 0.10), medium = quantile(New_cases, probs = 0.5), high = quantile(New_cases, probs = 0.90))
  
prevalence_sa <- prevalence %>% 
  filter(Country == "South Africa") %>% 
   group_by(week = floor_date(Date_reported, "week")) %>% 
              summarise(New_cases = sum(New_cases)) %>% 
   summarise(low = quantile(New_cases, probs = 0.10), medium = quantile(New_cases, probs = 0.5), high = quantile(New_cases, probs = 0.90))

I_ch_high <- unname(prevalence_ch$high/8654622)*n_ch
I_sa_high <- unname(prevalence_sa$high/59308690)*n_sa
I_tz_high <- unname(prevalence_sa$high/59308690)*n_tz
##high estimation
#I_ch_high <- (248788/8654622)*n_ch #https://covid19.who.int/region/euro/country/ch
#I_sa_high <- (162987/59308690)*n_sa
#I_tz_high <- (162987/59308690)*n_tz

#https://www.worldometers.info/world-population/population-by-country/ for the population

#medium estimation
#I_ch_medium <- (126868/8654622)*n_ch
#I_sa_medium <- (93978/59308690)*n_sa
#I_tz_medium <- (93978/59308690)*n_tz

I_ch_medium <- unname(prevalence_ch$medium/8654622)*n_ch
I_sa_medium<- unname(prevalence_sa$medium/59308690)*n_sa
I_tz_medium <- unname(prevalence_sa$medium/59308690)*n_tz

#low estimation
#I_ch_low <- (60000/8654622)*n_ch
#I_sa_low <- (45000/59308690)*n_sa
#I_tz_low <- (45000/59308690)*n_tz

I_ch_low <- unname(prevalence_ch$low/8654622)*n_ch
I_sa_low <- unname(prevalence_sa$low/59308690)*n_sa
I_tz_low <- unname(prevalence_sa$low/59308690)*n_tz

#fixed estimation (south africa)
I_ch_cov_fix <- unname(prevalence_sa$high/59308690)*n_ch
I_sa_cov_fix <- unname(prevalence_sa$high/59308690)*n_sa
I_tz_cov_fix <- unname(prevalence_sa$high/59308690)*n_tz
...
excess_mortality <- excess %>% 
  ...
  mutate(excess_weekly = mean_value/104) %>%  #excess mortality weekly
  mutate(lower_ci_weekly = lower_ci/104) %>% #lower bound
  mutate(upper_ci_weekly = upper_ci/104) #upper bound
  #mutate(cases_weekly = excess_weekly/IFR) %>% # excess mortality/IFR
  #mutate(cases_lower = lower_ci_weekly/IFR) %>% #lower bound
  #mutate(cases_upper = upper_ci_weekly/IFR) %>% #upper bound
  #mutate(SD = (cases_upper-cases_lower)/3.92)
...
sim_excess <- excess_mortality %>%
  ...
  mutate(excess_weekly = excess_weekly/IFR) %>% 
  mutate(sim_excess = rpois(n(), lambda = excess_weekly)) #i use IFR before the simulation to retain some variability, if not, i would just get values 0,100,200,300 ect. because poisson gives discrete output values.

I_covEX_ch <- sim_excess %>% 
  filter(country == "Switzerland") %>% 
  mutate(I = sim_excess/100000*n_ch)

I_covEX_tz <- sim_excess %>% 
  filter(country == "Tanzania") %>% 
  mutate(I = sim_excess/100000*n_tz)

I_covEX_sa <- sim_excess %>% 
  filter(country == "South Africa") %>% 
  mutate(I = sim_excess/100000*n_sa)

```

```{r plot other parameters}
#I plot is not necessary
I_data <- tibble(country= c(rep(c("Switzerland", "South Africa", "Tanzania"), 4)), I = c(I_ch,I_sa,I_tz, I_ch_high, I_sa_high, I_tz_high, I_ch_medium, I_sa_medium, I_tz_medium, I_ch_low, I_sa_low, I_tz_low), type = c("TB", "TB", "TB", "SARS (high)", "SARS (high)", "SARS (high)", "SARS (medium)", "SARS (medium)", "SARS (medium)", "SARS (low)", "SARS (low)", "SARS (low)" ))

I_data$type <- factor(I_data$type,
                      levels = c("TB", "SARS (high)", "SARS (medium)", "SARS (low)"))

I_data %>% 
  group_by (country, type)%>%
  ggplot +
  geom_col(aes(x=country, y= I, fill=type, color = country), position = position_dodge(1))+
  theme_bw()+
  theme(axis.title.x = element_blank())+
  labs(y = "Infectious persons per class") +
  scale_fill_brewer(palette = "pastel", name = NULL) +
  scale_color_manual(values = c("#B0C4DE", "#FFA07A", "#FFDEAD")) +
  guides(color = "none")
...
n_data %>% 
  ggplot +
  geom_col(aes(x=country, y= n, fill = country), position = position_dodge(1), width = 0.5)+ 
  theme_bw2()+
  theme(legend.position = "none")+
  theme(axis.title.x = element_blank()) +
  labs(y = "Class size") +
...
df_ch_COV_EX <- tibble(school = c(rep("Switzerland", 3000)), f = sample_f_ch, q_cov = sample_q_cov, I = I_covEX_ch$I) %>% 
  mutate("Excess" = 1 - exp(-(f*I*q_cov*month)/n_ch)) 

df_ch_COV <- tibble(school = c(rep("Switzerland", 3000)), f = sample_f_ch, q_cov = sample_q_cov) %>% 
  mutate("Fixed I/n" = 1 - exp(-(f*I_ch_cov_fix*q_cov*month)/n_ch)) %>% 
  mutate("High estimation" = 1 - exp(-(f*I_ch_high*q_cov*month)/n_ch)) %>% 
  mutate("Medium estimation" = 1 - exp(-(f*I_ch_medium*q_cov*month)/n_ch)) %>%
  mutate("Low estimation"= 1 - exp(-(f*I_ch_low*q_cov*month)/n_ch)) %>% 
  pivot_longer(
    cols = c("Fixed I/n", "High estimation", "Medium estimation", "Low estimation"),
    names_to = "Assumption",
    values_to = "risk")
#Tanzania
...
df_tz_COV_EX <- tibble(school = c(rep("Tanzania", 3000)), f = sample_f_tz, q_cov = sample_q_cov, I = I_covEX_tz$I) %>% 
  mutate("Excess" = 1 - exp(-(f*I*q_cov*month)/n_tz)) 

df_tz_COV <- tibble(school = c(rep("Tanzania", 3000)), f = sample_f_tz, q_cov = sample_q_cov) %>% 
  mutate("Fixed I/n" = 1 - exp(-(f*I_tz_cov_fix*q_cov*month)/n_tz)) %>% 
  mutate("High estimation" = 1 - exp(-(f*I_tz_high*q_cov*month)/n_tz)) %>% 
  mutate("Medium estimation"= 1 - exp(-(f*I_tz_medium*q_cov*month)/n_tz)) %>% 
  mutate("Low estimation"= 1 - exp(-(f*I_tz_low*q_cov*month)/n_tz)) %>% 
  pivot_longer(
    cols = c("Fixed I/n", "High estimation", "Medium estimation", "Low estimation"),
    names_to = "Assumption",
    values_to = "risk")
...
df_sa_COV_EX <- tibble(school = c(rep("South Africa", 3000)), f = sample_f_sa, q_cov = sample_q_cov, I = I_covEX_sa$I) %>% 
  mutate("Excess" = 1 - exp(-(f*I*q_cov*month)/n_sa)) 

df_sa_COV <- tibble(school = c(rep("South Africa", 3000)), f = sample_f_sa, q_cov = sample_q_cov) %>% 
  mutate("Fixed I/n" = 1 - exp(-(f*I_sa_cov_fix*q_cov*month)/n_sa)) %>% 
  mutate("High estimation" = 1 - exp(-(f*I_sa_high*q_cov*month)/n_sa)) %>% 
  mutate("Medium estimation" = 1 - exp(-(f*I_sa_medium*q_cov*month)/n_sa)) %>% 
  mutate("Low estimation" = 1 - exp(-(f*I_sa_low*q_cov*month)/n_sa)) %>% 
   pivot_longer(
    cols = c("Fixed I/n", "High estimation", "Medium estimation", "Low estimation"),
    names_to = "Assumption",
    values_to = "risk")
...
df_complet_COV_EX <- bind_rows(df_ch_COV_EX, df_tz_COV_EX, df_sa_COV_EX)
df_complet_COV <- bind_rows(df_ch_COV, df_sa_COV, df_tz_COV)

# mean+/-sd for Paper
##TB
df_ch_TB %>% 
  filter(Assumption == "Prevalence") %>% 
  summarise(mean = mean(risk$f), sd = sd(risk$f), median = median(risk$f), lower = quantile(risk$f, probs = 0.25), upper = quantile(risk$f, probs = 0.75), CI = quantile(risk$f, probs = c(0.025,0.975)))
...
#COV
df_ch_COV %>% 
  filter(Assumption == "Medium estimation") %>% 
  summarise(mean = mean(risk$f), sd = sd(risk$f), median = median(risk$f), lower = quantile(risk$f, probs = 0.25), upper = quantile(risk$f, probs = 0.75), CI = quantile(risk$f, probs = c(0.025,0.975)))
...
```{r plotting TB}
df_complet_TB %>% #all in 1 (not necessary)
  ggplot(aes(x = school, y=risk$f, fill = school, color = Assumption)) +
  ...
  guides(fill="none")+
 geom_point(aes(x = 1.925, y = quantile(df_ch_TB$risk$f[df_ch_TB$Assumption == "Fixed I/n"], probs = 0.95)), colour = "grey") +
   geom_point(aes(x = 2.075, y = quantile(df_ch_TB$risk$f[df_ch_TB$Assumption == "Prevalence"], probs = 0.95)), colour = "grey") +
   geom_point(aes(x = 0.925, y = quantile(df_sa_TB$risk$f[df_sa_TB$Assumption == "Fixed I/n"], probs = 0.95)), colour = "grey") +
   geom_point(aes(x = 1.075, y = quantile(df_sa_TB$risk$f[df_sa_TB$Assumption == "Prevalence"], probs = 0.95)), colour = "grey") +
   geom_point(aes(x = 2.925, y = quantile(df_tz_TB$risk$f[df_tz_TB$Assumption == "Fixed I/n"], probs = 0.95)), colour = "grey") +
   geom_point(aes(x = 3.075, y = quantile(df_tz_TB$risk$f[df_tz_TB$Assumption == "Prevalence"], probs = 0.95)), colour = "grey") +
  scale_fill_manual(values = c("#B0C4DE", "#FFA07A", "#FFDEAD")) +
  scale_color_manual(values = c("#DDA0DD","steelblue")) 

school_order <- c("Switzerland", "Tanzania", "South Africa")
...
p2 <- df_complet_TB %>% 
  ...
  guides(fill = "none") +
  geom_boxplot(width=0.3, alpha=0.2, outlier.shape = NA) +
  geom_point(aes(x=1,y=quantile(df_ch_TB$risk$f[df_ch_TB$Assumption == "Fixed I/n"], probs = 0.95)), colour="grey") +
  geom_point(aes(x=3,y=quantile(df_sa_TB$risk$f[df_sa_TB$Assumption == "Fixed I/n"], probs = 0.95)), colour="grey") +
  geom_point(aes(x=2,y=quantile(df_tz_TB$risk$f[df_tz_TB$Assumption == "Fixed I/n"], probs = 0.95)), colour="grey") +
...
p1 <- df_complet_TB %>% 
  ...
  guides(fill = "none") +
  geom_boxplot(width=0.3, alpha=0.2, outlier.shape = NA) +
  geom_point(aes(x=1,y=quantile(df_ch_TB$risk$f[df_ch_TB$Assumption == "Prevalence"], probs = 0.95)), colour="grey") +
  geom_point(aes(x=3,y=quantile(df_sa_TB$risk$f[df_sa_TB$Assumption == "Prevalence"], probs = 0.95)), colour="grey") +
 geom_point(aes(x=2,y=quantile(df_tz_TB$risk$f[df_tz_TB$Assumption == "Prevalence"], probs = 0.95)), colour="grey") +
  scale_fill_manual(values = c("#FFA07A", "#FFDEAD", "#B0C4DE")) +
...
df_complet_COV_EX %>% 
  ggplot(aes(x = school, y=Excess$f, fill = school)) +
  scale_y_continuous(labels = function(x) paste0(x*100), expand = c(0,0))+
  xlab("Country") + 
  ...
  geom_boxplot(width=0.3, alpha=0.2, outlier.shape = NA) +
  guides(fill="none")+
  geom_point(aes(x = 2, y = quantile(df_ch_COV_EX$Excess$f, probs = 0.05)), colour = "grey") +
  geom_point(aes(x = 2, y = quantile(df_ch_COV_EX$Excess$f, probs = 0.95)), colour = "grey") +
   geom_point(aes(x = 1, y = quantile(df_sa_COV_EX$Excess$f, probs = 0.05)), colour = "grey") +
   geom_point(aes(x = 1, y = quantile(df_sa_COV_EX$Excess$f, probs = 0.95)), colour = "grey") +
   geom_point(aes(x = 3, y = quantile(df_tz_COV_EX$Excess$f, probs = 0.05)), colour = "grey") +
   geom_point(aes(x = 3, y = quantile(df_tz_COV_EX$Excess$f, probs = 0.95)), colour = "grey") +
  scale_fill_manual(values = c("#B0C4DE", "#FFA07A", "#FFDEAD")) +
  scale_color_manual(values = c("#DDA0DD","steelblue")) +
...
```{r plotting SARS}
df_complet_COV %>% #all in 1 (not necessary)
  ggplot(aes(x = school, y=risk$f, fill = school, color = Assumption))+
  scale_y_continuous(labels = function(x) paste0(x*100))+
  xlab("Country") + 
  ylab("Monthly risk of infection (%)") +
  ...
  guides(fill = "none") +
  geom_boxplot(width=0.3, alpha=0.2, outlier.shape = NA) +
  #geom_point(aes(x=1,y=unname(quantile(df_sa$P_month_one_cov$f, probs = 0.95))),colour="grey") +
  #geom_point(aes(x=2,y=unname(quantile(df_ch$P_month_one_cov$f, probs = 0.95))),colour="grey") +
  #geom_point(aes(x=3,y=unname(quantile(df_tz$P_month_one_cov$f, probs = 0.95))),colour="grey") +
  scale_fill_manual(values = c("#B0C4DE", "#FFA07A", "#FFDEAD")) +
  scale_color_brewer(palette = "Pastel1") 

p3 <- df_complet_COV %>% 
  filter(Assumption == "High estimation") %>%  
  mutate(school = factor(school, levels = school_order)) %>%
  ggplot(aes(x = school, y=risk$f, fill = school))+
  scale_y_continuous(labels = function(x) paste0(x*100), expand = c(0,0))+
  xlab("Country") + 
  ...
  guides(fill = "none") + 
  geom_boxplot(width=0.3, alpha=0.2, outlier.shape = NA) +
  geom_point(aes(x=1,y=quantile(df_ch_COV$risk$f[df_ch_COV$Assumption == "High estimation"], probs = 0.95)), colour="grey") +
  geom_point(aes(x=3,y=quantile(df_sa_COV$risk$f[df_sa_COV$Assumption == "High estimation"], probs = 0.95)), colour="grey") +
  geom_point(aes(x=2,y=quantile(df_tz_COV$risk$f[df_tz_COV$Assumption == "High estimation"], probs = 0.95)), colour="grey") +
  scale_fill_manual(values = c("#FFA07A", "#FFDEAD", "#B0C4DE")) +
  ggtitle("High estimation") +
...
p4 <- df_complet_COV %>% 
  ...
  guides(fill = "none") + 
  geom_boxplot(width=0.3, alpha=0.2, outlier.shape = NA) +
  geom_point(aes(x=1,y=quantile(df_ch_COV$risk$f[df_ch_COV$Assumption == "Medium estimation"], probs = 0.95)), colour="grey") +
  geom_point(aes(x=3,y=quantile(df_sa_COV$risk$f[df_sa_COV$Assumption == "Medium estimation"], probs = 0.95)), colour="grey") +
  geom_point(aes(x=2,y=quantile(df_tz_COV$risk$f[df_tz_COV$Assumption == "Medium estimation"], probs = 0.95)), colour="grey") +
  scale_fill_manual(values = c("#FFA07A", "#FFDEAD", "#B0C4DE")) +
  ggtitle("Medium estimation") +
    theme(plot.title = element_text(hjust = 0.5)) +
  coord_cartesian(ylim = c(0, 0.2))

p5 <- df_complet_COV %>% 
  filter(Assumption == "Low estimation") %>%  
  mutate(school = factor(school, levels = school_order)) %>%
  ggplot(aes(x = school, y=risk$f, fill = school))+
  scale_y_continuous(labels = function(x) paste0(x*100), expand = c(0,0))+
  xlab("Country") + 
  ...
  guides(fill = "none") + 
  geom_boxplot(width=0.3, alpha=0.2, outlier.shape = NA) +
  geom_point(aes(x=1,y=quantile(df_ch_COV$risk$f[df_ch_COV$Assumption == "Low estimation"], probs = 0.95)), colour="grey") +
  geom_point(aes(x=3,y=quantile(df_sa_COV$risk$f[df_sa_COV$Assumption == "Low estimation"], probs = 0.95)), colour="grey") +
  geom_point(aes(x=2,y=quantile(df_tz_COV$risk$f[df_tz_COV$Assumption == "Low estimation"], probs = 0.95)), colour="grey") +
  scale_fill_manual(values = c("#FFA07A", "#FFDEAD", "#B0C4DE")) +
  ggtitle("Low estimation") +
    theme(plot.title = element_text(hjust = 0.5)) +
  coord_cartesian(ylim = c(0, 0.2))
...
p6 <- df_complet_COV %>% 
  ...
  guides(fill = "none") + 
  geom_boxplot(width=0.3, alpha=0.2, outlier.shape = NA) +
  geom_point(aes(x=1,y=quantile(df_ch_COV$risk$f[df_ch_COV$Assumption == "Fixed I/n"], probs = 0.95)), colour="grey") +
  geom_point(aes(x=3,y=quantile(df_sa_COV$risk$f[df_sa_COV$Assumption == "Fixed I/n"], probs = 0.95)), colour="grey") +
  geom_point(aes(x=2,y=quantile(df_tz_COV$risk$f[df_tz_COV$Assumption == "Fixed I/n"], probs = 0.95)), colour="grey") +
  scale_fill_manual(values = c("#FFA07A", "#FFDEAD", "#B0C4DE")) +
  ggtitle("Fixed I/n estimation") +
    theme(plot.title = element_text(hjust = 0.5)) +
  coord_cartesian(ylim = c(0, 0.2))
  
plot_grid(p3, p4, p5, p6, ncol = 2) #plot in the paper
```
