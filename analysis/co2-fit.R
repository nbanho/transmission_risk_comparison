...
means <- format(round(c(mean(co2.sa$co2), mean(co2.ch$co2), mean(co2.tz$co2))), big.mark = ",")

plot.co2.raw <- rbind(
  dplyr::select(co2.sa, country, co2),
  dplyr::select(co2.ch, country, co2),
  dplyr::select(co2.tz, country, co2)
) %>%
  mutate(country = factor(country, levels = c("South Africa", "Switzerland", "Tanzania"))) %>%
  mutate(co2 = ifelse(co2 > 4000, 4000, co2),
         co2 = ifelse(co2 < 400, 400, co2)) %>%
  ggplot(aes(x = co2, y = ..density.., fill = country)) +
  facet_wrap(~ country, scales = "free_y") +
  geom_histogram(bins = 30, color = "black") +
  geom_text(data = data.frame(x = 2000, 
                              y = c(7e-04, 7.2e-04, 0.00375),
